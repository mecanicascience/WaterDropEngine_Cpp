	...
	public:
		...
		static void createBuffer(VkPhysicalDevice &physicalDevice, VkDevice &device, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer &buffer, VkDeviceMemory &bufferMemory) {
			...
			if (vkCreateBuffer(device, &bufferInfo, nullptr, &buffer) != VK_SUCCESS)
				throw WdeException(LogChannel::RENDER, "Failed to create buffer.");
			...
			if (vkAllocateMemory(device, &allocInfo, nullptr, &bufferMemory) != VK_SUCCESS)
				throw WdeException(LogChannel::RENDER, "Failed to allocate buffer memory.");
		...
		static uint32_t findMemoryType(VkPhysicalDevice &physicalDevice, uint32_t typeFilter, VkMemoryPropertyFlags properties) {
			...
			throw WdeException(LogChannel::RENDER, "Failed to find suitable memory type.");
		}
	};
}
